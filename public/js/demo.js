...
function allReady(thresholds, sampleText) {
  var $input = $(".input"),
    ...
    originalTextDescription_template =
      originalTextDescriptionTemplate.innerHTML, // eslint-disable-line camelcase
  ...
  function toneCallback(data) {
    ...
    if (data.document_tone.warning != null) {
      $documentWarning.html(
        _.template(documentWarning_template, {
    ...
    function emotionMap(item) {
      ...
      return {
        ...
        thresholdHigh: app.percentagify(
          app.thresholds().doc[item.tone_name][1]
        ),
      };
    }
    ...
    function matchSentenceViewsHeight() {
      $(".sentences--sentence-views").css(
        "height",
        $(".sentences--filters")[0].getBoundingClientRect().height + "px"
      );
    }
    ...
    function updateFilters() {
      ...
      if (app.selectedFilter() == "No Tone") {
        $(
          ".filters--radio[data-id=" + normalize(app.selectedFilter()) + "]"
        ).prop("checked", true);
      } else {
        $(".filters--radio[data-id=" + app.selectedFilter() + "]").prop(
          "checked",
          true
        );
      }
    ...
    function updateOriginalText() {
      $originalTexts.html(
        _.template(originalText_template, {
          items: app.updateOriginalSentences(),
        })
      );

      $originalTextDescription.html(
        _.template(originalTextDescription_template, {
          items: [
            {
    ...
    function updateSentenceRank() {
      $sentenceRankTable.html(
        _.template(sentenceRank_template, {
    ...
    function updateOriginalTextTooltip(index) {
      $originalTextTooltipContainer.html(
        _.template(originalTextTooltip_template, {
    ...
    function updateLegend() {
      if (sentenceTone.length > 0) {
        $legend.html(
          _.template(originalTextLegend_template, {
    ...
    function bindOriginalTextHoverEvents() {
      ...
      $("body").click(function (e) {
        if (!$(e.target).hasClass("original-text--sentence-container")) {
          $originalTextTooltipContainer.addClass(
            "original-text--tooltip-container_hidden"
          );
        }
      });
    ...
    if (
      typeof data.document_tone.tones !== "undefined" &&
    ...
    if (
      ...
      $emotionGraph.html(
        _.template(emotionBarGraph_template, {
          items: [
            {
              label: "No Tone",
  ...
  function _error(error) {
    ...
    if (error.responseJSON.code === 429) {
      message =
        "You've sent a lot of requests in a short amount of time. " +
        "As the CPU cores cool off a bit, wait a few seonds before sending more requests.";
    }

    $errorMessage.html(
      _.template(errorMessage_template, {
        items: [
          {
  ...
  function getToneAnalysis(text) {
    $.post(
      "/api/tone",
      { toneInput: { text: text }, contentLanguage: selectedLang },
      toneCallback
    ).fail(_error);
  }
  ...
  $inputRadio.click(function () {
    ...
    $("input:radio[name=rb][value=" + $(this).val() + "]").prop(
      "checked",
      true
    );
    ...
    if (selectedInputSample === "own-text") {
      ...
      $displayInputRadioLang.removeClass(
        "original-text--tooltip-container_hidden"
      );
    } else {
      $displayInputRadioLang.addClass(
        "original-text--tooltip-container_hidden"
      );
  ...
  $inputRadioLang.click(function () {
    ...
    $("input:radio[name=rb-lang][value=" + $(this).val() + "]").prop(
      "checked",
      true
    );
  });
